...
const NavMenuElement = ({ url, title }: NavMenuElementProps) => {
  return (
    <a href={url} className="flex flex-col group mx-3 cursor-pointer">
      ...
      <div className="h-[2px] flex w-5 opacity-0 bg-white group-hover:w-full group-hover:opacity-100 transition-all duration-300 group-focus:opacity-100  group-focus:w-full"></div>
    </a>
  );
};
